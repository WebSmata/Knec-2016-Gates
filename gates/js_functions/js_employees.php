<?php
	function js_signin() {
		...
		$loginkey = md5($_POST['password']);
			...
			$_SESSION['gates_user'] = js_let_me_user($loginname, $loginkey);
			$_SESSION['gates_account'] = js_logged_account($loginname);
	...
	function js_is_logged(){
		$myloginid = isset( $_SESSION['gates_user'] ) ? $_SESSION['gates_user'] : "";
	...
	function js_signin_modal() {
	  if ( isset( $_POST['LetMeIn'] ) ) {
		...
		if( $database->js_num_rows( $js_db_query ) > 0 ) {
		    ...
		    $_SESSION['gates_user'] = $employeeid;			
...
function logout() {
  unset( $_SESSION['gates_user'] );
  unset( $_SESSION['gates_account'] );
  header( "Location: index.php" );
}
